...
contract EVMAccountsTest {
    ...
    function testGetEmpty() public payable returns (bool ret) {
        ...
        EVMAccounts.Account memory acc = accs.get(address(0x01));
        assert(accs.size == 1);
        assert(acc.addr == address(0x01));
    }
    ...
    function testGetTwoNew() public payable returns (bool ret) {
        ...
        EVMAccounts.Accounts memory accs;
        EVMAccounts.Account memory acc = accs.get(address(0x01));
        EVMAccounts.Account memory acc2 = accs.get(address(0x02));
        assert(accs.size == 2);
        assert(acc.addr == address(0x01));
        assert(acc2.addr == address(0x02));
    }

    function testGetAlreadyExisting() public payable returns (bool ret) {
        ret = true;
        EVMAccounts.Accounts memory accs;
        accs.get(address(0x01));
        EVMAccounts.Account memory acc = accs.get(address(0x01));
        assert(accs.size == 1);
        assert(acc.addr == address(0x01));
    }
    ...
    function testCopy() public payable returns (bool ret) {
        ...
        EVMAccounts.Account memory cAcc = accs.get(address(0x01));
        EVMAccounts.Account memory cAcc2 = accs.get(address(0x02));

        assert(cpy.size == 2);

        assert(cAcc.addr == address(0x01));
        ...
        assert(cAcc2.addr == address(0x02));
